...
public class LoginAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		if (memberService.checkLogin(member)) {
			...
			String memberName = memberService.getMemberById(id).getName();
			context.getSession().put("memberName", memberName);
			
			if (member_state.equals("stop")||member_state.equals("cancel")) {
				out.print("<script>window.location.href='index.jsp?nomember=1'</script>");
			} else {
			...
			out.print("<script>window.location.href='index_w.jsp'</script>");
			...
			out.print("<script>window.location.href='index_m.jsp'</script>");
		} else {
